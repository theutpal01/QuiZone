		...
		return new Promise((resolve, reject) => {
			fileReader.onload = async (e) => {
				...
				try {
					...
					const rows = worksheet.getSheetValues(); // 1-indexed rows and cols
					...
					// safeTrim: convert undefined/null to '', trim strings, keep numbers as string
					const safeTrim = (cell) => {
						if (cell === undefined || cell === null) return '';
						return String(cell).trim();
					};

					// start at row 2 (assume row 1 is header). If your sheet has no header, set startIndex = 1
					const startIndex = 2;

					// helper: map answer index/letter to the actual choice text
					const mapIndexToChoice = (rawAns, choices) => {
						const a = safeTrim(rawAns);
						if (!a) return '';

						// if raw answer exactly equals one of choices, return it
						for (const ch of choices) {
							if (ch && ch.toLowerCase() === a.toLowerCase()) return ch;
						}
						...
						// letter index A..D or a..d
						if (/^[a-dA-D]$/.test(a)) {
							const idx = a.toUpperCase().charCodeAt(0) - 'A'.charCodeAt(0);
							return choices[idx] || '';
						}

						// otherwise, return raw text (best-effort)
						return a;
					};

					// sequentially process rows to avoid state flooding
					for (let r = startIndex; r < rows.length; r++) {
						const row = rows[r];
						if (!row) continue;

						// According to your strict layout:
						// Column 1 -> unused (maybe course code), Column 2 -> question, 3..6 -> options, 7 -> answer
						const questionCell = safeTrim(row[2]);
						const option1 = safeTrim(row[3]);
						const option2 = safeTrim(row[4]);
						const option3 = safeTrim(row[5]);
						const option4 = safeTrim(row[6]);
						const rawAnswer = safeTrim(row[7]);

						// Skip row if question empty
						if (!questionCell) continue;

						// Skip row if any choice missing (strict)
						if (!option1 || !option2 || !option3 || !option4) {
							// If you prefer to allow missing choices, remove this continue and handle accordingly.
							console.warn(`Skipping row ${r} - not enough choices.`);
							continue;
						}

						const choices = [option1, option2, option3, option4];

						// Map answer index/letter/text to actual choice text
						const finalAnswer = mapIndexToChoice(rawAnswer, choices);

						// If finalAnswer is still empty, skip (strict). Change behavior if you want to accept blank answers.
						if (!finalAnswer) {
							console.warn(`Skipping row ${r} - unable to determine answer.`);
							continue;
						}

						// call addQuestion(questionText, choice1, choice2, choice3, choice4, answer)
						try {
							const maybePromise = addQuestion(questionCell, option1, option2, option3, option4, finalAnswer);
							if (maybePromise && typeof maybePromise.then === 'function') {
								await maybePromise;
							}
						} catch (err) {
							console.warn(`Error adding question from row ${r}:`, err);
							// continue to next row even if addQuestion fails for this one
						}
					}
